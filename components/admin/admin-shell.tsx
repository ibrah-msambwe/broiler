...
export default function AdminShell({ active, onSelect, children, menuItems }: AdminShellProps) {
              ...
              <SidebarMenu className="gap-2 md:gap-3">
                {items.map((m) => (
                  <SidebarMenuItem key={m.id}>
                    <SidebarMenuButton isActive={active === m.id} onClick={() => onSelect(m.id)} className="group data-[active=true]:shadow-sm md:h-10" size="sm">
        ...
        <div className="sticky top-0 z-20 bg-white border-b">
          <div className="h-16 px-4 flex items-center gap-3">
            ...
            <div className="text-lg md:text-2xl font-extrabold relative select-none tracking-wide" style={{fontFamily: 'Inter, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial'}}>
              <span className="absolute inset-0 blur-md opacity-40 bg-clip-text text-transparent bg-gradient-to-r from-slate-500 to-slate-700">TARIQ BROILER MANAGEMENT</span>
              <span className="absolute inset-0 blur-sm opacity-50 bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 via-sky-500 to-emerald-500">TARIQ BROILER MANAGEMENT</span>
              <span className="relative z-10 bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 via-sky-600 to-emerald-600" style={{textShadow: "2px 2px 4px rgba(0,0,0,0.25)"}}>TARIQ BROILER MANAGEMENT</span>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <Button size="sm" className="hidden sm:inline-flex">EN</Button>
              <Button size="sm" className="hidden sm:inline-flex">SW</Button>
