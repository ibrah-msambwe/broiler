...
interface AdminCommunicationSystemProps {
  currentUserId: string
  currentUserName: string
  isAdmin: boolean
}

export default function AdminCommunicationSystem({ 
  currentUserId, 
  currentUserName, 
  isAdmin 
}: AdminCommunicationSystemProps) {
  ...
  const loadUsers = async () => {
    try {
      setIsLoading(true)
      console.log("👥 Loading users for admin communication...")
      ...
      try {
        ...
        if (batchesError) {
          console.log("❌ Batches table error:", batchesError.message)
      ...
      try {
        ...
        if (profilesError) {
          console.log("❌ Profile table error:", profilesError.message)
      ...
      if (allUsers.length === 0) {
        console.log("📝 No users found in database, creating mock users for demo")
      ...
      // Auto-select first user if admin
      if (isAdmin && uniqueUsers.length > 0 && !selectedUser) {
